...
class HealthObserver: NSObject, FlutterStreamHandler {
    ...
    func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        ...
        guard let args = arguments as? [String: Any],
            let dataTypeKeys = args["dataTypeKeys"] as? [String]
        else {
            return FlutterError(
    ...
    private func startObserving(dataTypeKey: String, enableBackground: Bool) -> FlutterError? {
        ...
        let query = HKObserverQuery(sampleType: sampleType, predicate: nil) { [weak self]
        ...
        if enableBackground {
            ...
            healthStore.enableBackgroundDelivery(for: sampleType, frequency: frequency) {
                success, error in
                if let error = error {
                    self.sendEvent([
